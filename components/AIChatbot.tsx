...
export const AIChatbot: React.FC<AIChatbotProps> = ({ userData, apiKey, onClose, scriptUrl, isAdmin = false }) => {
  ...
  // [수정] 키 경고 멘트도 '조금 다른 분' 대신 '완벽히 일치하지 않을 수 있음'으로 순화
  const REACTION_HEIGHT_WARNING = "(중요: 키는 보장 조건이 아니므로, '선호하시니 맞춰보겠지만 필수 조건은 아니라서 상황에 따라 키 조건이 완벽히 일치하지 않을 수도 있다는 점 참고 부탁드립니다'라는 취지의 안내를 반드시 포함하세요)";
  ...
  const systemInstruction = `
    ...
    - 반응: 기계적인 반복을 피하기 위해 아래 3가지 멘트 중 하나를 자연스럽게 골라서 사용하세요. (단, '조금 다른 분' 같은 표현은 흡연 여부 등에서 어색하므로 사용하지 마세요.)
      옵션 1: "네, 말씀하신 내용은 최우선으로 반영하겠습니다! 다만 필수 보장 조건은 아니다 보니, 다른 조건들이 이상형에 가깝다면 부득이하게 해당 부분은 완벽히 일치하지 않을 수도 있다는 점 미리 양해 부탁드립니다."
      옵션 2: "넵! 선호하시는 대로 최대한 맞춰보겠습니다. 다만 보장 조건은 아니어서 100% 확답드리기는 어렵지만, 신경 써서 꼼꼼히 찾아볼게요!"
      옵션 3: "알겠습니다. 최대한 반영해 보겠지만, 전체적인 매칭 퀄리티를 위해 매칭풀 상황에 따라 조금 유연하게 진행될 수도 있다는 점 참고해 주세요!"
